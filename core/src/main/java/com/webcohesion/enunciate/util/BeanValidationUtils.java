...
import com.webcohesion.enunciate.beanval.BeanValidationContext;
import com.webcohesion.enunciate.beanval.NotNullVisitor;
  ...
  private BeanValidationUtils() {}
    ...
    private static boolean isNotNull(Element el, boolean recurse) {
        ...
        if (el.getAnnotation(NotNull.class) != null) {
            return applyValidationGroups(el);
        }
    ...
    private static boolean applyValidationGroups(Element el) {

        NotNullVisitor<Void, BeanValidationContext> notNullVisitor = new NotNullVisitor();

        el.accept(notNullVisitor, null);

        return notNullVisitor.isRequired();
    }
}
